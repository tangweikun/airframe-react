								...
								>
									<Avatar.Image
										...
										addOns={[
											...
											<AvatarAddOn.Icon className="fa fa-circle" color={item.status} key="avatar-icon-fg" />
										]}
									/>
								</div>
							)}
...
export class HorizontalLists extends React.Component {
	...
	onDragEnd(result) {
		...
		if (source.droppableId === destination.droppableId) {
			const items = reorder(this.state[`${source.droppableId}Items`], source.index, destination.index);
		...
		return (
			<div className={className}>
				<DragDropContext onDragEnd={this.onDragEnd}>
					...
					<RowList listId="listB" items={this.state.listBItems} title="Candidates Interview" className="mt-4" />
					<RowList listId="listC" items={this.state.listCItems} title="Candidates Testing" className="mt-4" />
				</DragDropContext>
			</div>
		);
	}
