...
export class AdvancedTableA extends React.Component {
	...
	handleDeleteRow() {
		this.setState({
			products: _.filter(this.state.products, product => !_.includes(this.state.selected, product.id))
		});
	}
	...
	render() {
		...
		const paginationDef = paginationFactory({
			paginationSize: 5,
			showTotal: true,
			pageListRenderer: props => <CustomPaginationPanel {...props} size="sm" className="ml-md-auto mt-2 mt-md-0" />,
			sizePerPageRenderer: props => <CustomSizePerPageButton {...props} />,
			paginationTotalRenderer: (from, to, size) => <CustomPaginationTotal {...{ from, to, size }} />
		});
		...
		return (
			<ToolkitProvider keyField="id" data={this.state.products} columns={columnDefs} search exportCSV>
				{props => (
					<React.Fragment>
