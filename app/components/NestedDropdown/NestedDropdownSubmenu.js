...
class NestedDropdownSubmenu extends React.Component {
	...
	render() {
		const { tag: Tag, subMenuTag: SubMenuTag, title, children, className, openId, onOpen } = this.props;
		...
		return (
			<Tag className={itemClass}>
				...
				<div className="nested-dropdown__submenu-item__menu-wrap">
					<SubMenuTag className="nested-dropdown__submenu-item__menu dropdown-menu">{children}</SubMenuTag>
				</div>
			</Tag>
		);
	}
...
const ContextNestedDropdownSubmenu = props => (
	<Consumer>{contextProps => <NestedDropdownSubmenu {...contextProps} {...props} />}</Consumer>
);
