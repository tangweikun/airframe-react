...
const getCurrentbreakPoint = (width, breakPoints) => {
	...
	for (let bp of breakPoints) {
		if ((_.isUndefined(bp.min) || bp.min <= width) && (_.isUndefined(bp.max) || bp.max > width)) {
			output = bp.id;
		}
	}
	return output;
...
export class Col extends React.Component {
	...
	render() {
		...
		const floatColClasses = classNames(className, "float-col", "float-column", `float-column--size-${floatColBpId}`);
		...
		return active ? (
			...
			<BootstrapCol {..._.extend(bsColumnProps, otherProps)} className={classNames(className, "pb-3")}>
				{children}
			</BootstrapCol>
		);
	}
