	...
	render() {
		...
		if (this.context.active) {
			...
			const adjustedChildren = simplifyChildrenArray(
				React.Children.map(children, child => React.cloneElement(child, { trueSize: trueColSizes[child.props.i] }))
			);
			...
			for (let child of childrenArray) {
				...
				bpData = Object.assign(bpData, {
					...
					...{
						...
						x: _.isUndefined(child.props[`${breakPoint}X`]) ? rowCounter : child.props[`${breakPoint}X`],
					...
					// props if defined for controlled type
					y: _.isUndefined(child.props[`${breakPoint}Y`]) ? y : child.props[`${breakPoint}Y`]
				});
