...
export class VersionSelector extends React.Component {
	...
	async fetchVersions() {
		...
		let versions;
		try {
			versions = await fetch(`${SERVICE_URL}/dashboards/versions`).then(response => response.json());
		} catch (exc) {
				...
				{!_.isEmpty(versions) && (
					<DropdownMenu>
						...
						{_.map(versions, (version, index) => (
							<DropdownItem key={index} href={version.demoUrl} className="d-flex" active={version === currentVersion}>
								<span>
									{version.label} {version.version}
									<br />
									<span className="small">{moment(version.date).format("ddd, MMM DD, YYYY h:mm:ss A")}</span>
								</span>
