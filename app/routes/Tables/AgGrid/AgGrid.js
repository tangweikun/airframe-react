...
class SkillFilter {
	...
	doesFilterPass({ data }) {
		...
		const passed = reduce(IT_SKILLS, (acc, skill) => acc || (rowSkills[skill] && model[skill]), false);

		return passed;
	}
	isFilterActive() {
		return this.model.android || this.model.css || this.model.html5 || this.model.mac || this.model.windows;
	}
}
	...
	render() {
		...
		return (
			<Container>
				...
				<p className="pb-3">
					<strong>Over 2,500 Companies use ag-Grid.</strong> The &quot;ag&quot; part of ag-Grid stands for
					&quot;agnostic&quot;. The internal ag-Grid engine is implemented in TypeScript with zero dependencies. ag-Grid
					supports Angular through a wrapper component. The wrapper lets you use ag-Grid in your application like any
					other Angular component – you pass configuration through property bindings and handle events through event
					bindings. You can even use Angular components to customize the grid UI and cell contents / behavior.
				</p>
					...
					<div className="ag-theme-bootstrap" style={{ height: "600px" }}>
						...
						>
							...
							<AgGridColumn headerName="Employee">
								<AgGridColumn headerName="Name" field="name" width={150} cellRenderer={nameRenderer} />
								<AgGridColumn
									headerName="Country"
									field="country"
									width={150}
							...
							<AgGridColumn headerName="Contact">
								<AgGridColumn headerName="Mobile" field="mobile" width={180} filter="agTextColumnFilter" />
								<AgGridColumn headerName="Land-line" field="landline" width={180} filter="agTextColumnFilter" />
								<AgGridColumn headerName="Address" field="address" width={180} filter="agTextColumnFilter" />
							</AgGridColumn>
						</AgGridReact>
					</div>
